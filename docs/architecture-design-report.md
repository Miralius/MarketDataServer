...
| `PollingScheduler`      | Scheduler: Планирует polling и повтор transient-операций через `boost::asio::steady_timer`                                    |
...
| `transient_retry_count` | Число последовательных transient retries текущей provider-операции |
...
sequenceDiagram
    ...
    Worker->>Scheduler: ScheduleAfterPollInterval
...
Временная сетевая или транспортная ошибка операции поставщика, включая gRPC `UNAVAILABLE` и эквивалентные ошибки адаптера, не завершает задание сразу. `PollingScheduler` планирует повтор той же операции через `poll_interval`; worker-поток во время ожидания не блокируется. Параметр `max_transient_retries` ограничивает число повторных попыток подряд. Значения по умолчанию: `poll_interval = 1 s`, `max_transient_retries = 3`.

Счётчик ведётся для последовательных transient-ошибок и сбрасывается после любого успешного ответа поставщика. Общий deadline задания всегда имеет приоритет над retry budget. Если deadline ещё не истёк, четвёртая последовательная transient-ошибка (исходная попытка и три retry) переводит задание в `FAILED` с `ERROR_CODE_PROVIDER_UNAVAILABLE`.

При transient-ошибке во время потокового получения ответа уже сохранённые чанки удаляются, а повторное чтение начинается с chunk 0. Нарушение содержимого протокола и permanent provider errors не повторяются. Повтор отправки запроса должен сохранять тот же идемпотентный ключ, чтобы retry не создавал у поставщика вторую операцию.

На cache hit новое обращение к поставщику не выполняется: клиент получает идентификатор уже завершённого задания. Если очередь заполнена, `Submit` завершается с `RESOURCE_EXHAUSTED`, а не запущенное задание не остаётся в состоянии `QUEUED`.
...
## 7. Runtime-конфигурация retry

| Параметр                | Значение по умолчанию | Назначение                                                                       |
|-------------------------|----------------------:|----------------------------------------------------------------------------------|
| `poll_interval`         |                 `1 s` | Интервал polling и повторов transient-операций                                   |
| `max_transient_retries` |                   `3` | Максимум повторных попыток подряд; успешный provider response сбрасывает счётчик |

Оба параметра задаются без перекомпиляции и валидируются при запуске.
