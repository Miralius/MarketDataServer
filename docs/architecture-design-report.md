...
|-------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|
| `jobs`            | внутренний `job_handle BIGINT`, публичный `job_id UUID`, исходный request, `status`, `cache_key`, provider request id, ошибка и timestamps           |
| `response_chunks` | `job_handle`, `chunk_no`, `payload BYTEA`; PK `(job_handle, chunk_no)`, payload учитывает protobuf overhead, чтобы `ResultChunk` был не больше 1 MiB |
